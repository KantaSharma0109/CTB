...
class CartItem {
  ...
  factory CartItem.fromJson(Map<String, dynamic> jsonData) {
    return CartItem(
      cart_id: jsonData['id'].toString(),
      name: jsonData['name'].toString(),
      item_category: jsonData['category'].toString(),
      price: int.parse(jsonData['discount_price'].toString()),
      cart_category: jsonData['cart_category'] != null
          ? jsonData['cart_category'].toString()
          : 'cart',
      image_path: jsonData['image_path']?.toString() ?? '',
      item_id: jsonData['item_id'].toString(),
      quantity: int.parse(jsonData['quantity'].toString()),
      totalAmount: jsonData['totalAmount'] != null
          ? int.parse(jsonData['totalAmount'].toString())
          : 0,
      address: jsonData['address']?.toString() ?? '',
      description: jsonData['description']?.toString() ?? '',
      pincode: jsonData['pincode']?.toString() ?? '',
      isPlusMinus: jsonData['isPlusMinus'] ?? false,
    );
  }
}
