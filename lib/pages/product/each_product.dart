...
class _EachProductState extends State<EachProduct>
  ...
  Future<void> addReviewsByCategory() async {
    ...
    Map<String, dynamic> requestBody = {
      ...
      'user_id': Application.userId,
      'message': _message.text,
      'category': 'product', // This could be 'course', 'product', or 'book'
      'item_id': widget
  ...
  Future<void> getReviewsByCategory() async {
    ...
    Map<String, dynamic> requestData = {
      'category': 'product', // or 'course' or 'book' depending on the category
  ...
  Future<void> getProductImages() async {
    ...
    // String urlToSend = '$url/getProductImages/${widget.id}';
    // print('Sending request to: $urlToSend'); // This will print the full URL
    // Map<String, dynamic> _getNews = await MySqlDBService().runQuery(
    //   requestType: RequestType.GET,
    //   url: urlToSend,
    // );
    String completeUrl =
        ...
        '${Constants.baseUrl}getProductImg.php?product_id=${widget.id}';
...
// Make the API call
  ...
  Future<void> getProduct() async {
    try {
      final url = Uri.parse('${Constants.baseUrl}each_product.php');
      ...
      if (response.statusCode == 200) {
        ...
        if (_getNews.containsKey('product')) {
          final _data = _getNews['product'];
          setState(() {
            name = _data['name'].toString();
            description = _data['description'].toString();
            category = _data['c_name'].toString();
            price = _data['price'].toString();
            discount_price = _data['discount_price'].toString();
            relatedProducts = _data['related_products_array'].toString();
            share_url = _data['share_url'].toString();
          });
          getProductImages(); // Fetch product images
        } else if (_getNews.containsKey('error')) {
          Utility.printLog(_getNews['error']);
          Utility.showProgress(false);
        }
      } else {
        Utility.printLog('Server returned an error: ${response.statusCode}');
        Utility.showProgress(false);
      }
    } catch (e) {
      Utility.printLog('Error occurred: $e');
      Utility.showProgress(false);
    }
  }
                                          ...
                                          if (price != discount_price)
                                            Padding(
                                              padding: const EdgeInsets.only(
                                                  bottom: 3.0),
                                              child: Text(
                                                '${(((int.parse(price) - int.parse(discount_price)) / int.parse(price)) * 100).toStringAsFixed(2)}%',
                                                style: const TextStyle(
                                                  color: Palette.secondaryColor,
                                                  fontSize: 16.0,
                                                  fontFamily:
